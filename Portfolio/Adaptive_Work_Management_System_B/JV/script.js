document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE (状態管理) ---
    // ReactのuseStateに相当。アプリケーションの状態をここにまとめる。
    ...
    let userProfile = JSON.parse(localStorage.getItem('userProfile')) || {
        ...
        bestWorkingHours: [9, 10, 11, 14, 15] // 24時間制
    };
    ...
    const calculateRecommendedTime = (difficulty) => {
        ...
        const multipliers = {
            'very-easy': 0.5, 'easy': 0.7, 'medium': 1.0, 'hard': 1.5, 'very-hard': 2.0
        };
    ...
    const startTask = (taskId) => {
        tasks = tasks.map(task => 
            task.id === taskId 
                ? { ...task, status: 'in-progress', startTime: new Date().toISOString() } 
                : task
        );
        saveState();
        render();
    ...
    const completeTask = (taskId) => {
        ...
        tasks = tasks.map(task => {
            if (task.id === taskId) {
                const startTime = task.startTime ? new Date(task.startTime) : new Date(task.createdAt);
                ...
                completedTaskInstance = {
                    ...
                    actualTime: actualTime || task.recommendedTime, // 0分なら推奨時間を採用
                };
        ...
        if (completedTaskInstance) {
            updateUserProfile(completedTaskInstance);
        }
    ...
    const updateUserProfile = (completedTask) => {
        ...
        const newAverageTime = totalCompleted > 0 ? Math.round(
            allCompletedTasks.reduce((sum, t) => sum + t.actualTime, 0) / totalCompleted
        ) : userProfile.averageTaskTime;

        userProfile = {
            ...userProfile,
            completionRate: isNaN(newCompletionRate) ? userProfile.completionRate : newCompletionRate,
            averageTaskTime: isNaN(newAverageTime) ? userProfile.averageTaskTime : newAverageTime
        };
    };
    ...
    const saveProfileSettings = (e) => {
        ...
        const hoursString = bestHoursInput.value.trim();
        userProfile.bestWorkingHours = hoursString
            .split(',')
            .map(s => Number(s.trim()))
    ...
    // --- RENDER (描画関数) ---
    // Reactのレンダリング部分に相当。状態が変わるたびに呼び出され、画面を再構築する。
    ...
    const renderTaskLists = () => {
        ...
        tasks.forEach(task => {
            ...
            if (task.status === 'pending') {
                pendingTasksList.innerHTML += taskCardHTML;
            } else if (task.status === 'in-progress') {
                inprogressTasksList.innerHTML += taskCardHTML;
            } else {
                completedTasksList.innerHTML = taskCardHTML + completedTasksList.innerHTML;
            }
        });
    };
    ...
    const createTaskCard = (task) => {
        ...
        if (task.status === 'pending') {
            return `
                <div class="task-card ${difficultyClass}" data-task-id="${task.id}">
                    <h4>${task.title}</h4>
                    <p>${task.description || ' '}</p>
                    <div class="task-meta">
                        <div class="task-meta-left">
                            <i data-lucide="clock"></i>
                            <span>推奨: ${task.recommendedTime}分</span>
                            <span class="difficulty-badge">${task.difficulty}</span>
                        </div>
                        <button class="btn btn-secondary start-btn">開始</button>
                    </div>
                </div>`;
        }
        if (task.status === 'in-progress') {
            ...
            return `
                <div class="task-card task-inprogress ${difficultyClass}" data-task-id="${task.id}">
                    <h4>${task.title}</h4>
                    <p>${task.description || ' '}</p>
                    <div class="task-inprogress-meta">
                        <span>経過: ${elapsed}分 / ${task.recommendedTime}分</span>
                        <div class="progress-bar-container">
                            <div class="progress-bar-fill" style="width: ${progress}%"></div>
                        </div>
                    </div>
                    <button class="btn btn-complete complete-btn">完了</button>
                </div>`;
        }
        if (task.status === 'completed') {
            return `
                <div class="task-card task-completed" data-task-id="${task.id}">
                    <h4><i data-lucide="check-circle"></i> ${task.title}</h4>
                    <p>${task.description || ' '}</p>
                    <div class="actual-time">
                        実際の時間: ${task.actualTime}分 (推奨: ${task.recommendedTime}分)
                    </div>
                </div>`;
        }
        return '';
    ...
    const renderAnalytics = () => {
        ...
        document.getElementById('profile-completion').textContent = `${Math.round(userProfile.completionRate * 100)}%`;
        ...
        document.getElementById('suggestion-pattern').textContent = `あなたの完了率は${Math.round(userProfile.completionRate * 100)}%です。平均作業時間は${userProfile.averageTaskTime}分で、${userProfile.bestWorkingHours.join('時、')}時の時間帯で最も効率的に作業されています。`;
